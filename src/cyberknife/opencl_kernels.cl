/**
 * OpenCL kernels for CyberKnife dose calculation
 *
 ...
 */
float interpolate2D(__global const float* table,
                    __global const float* xValues,
                    __global const float* yValues,
                    int xCount,
                    int yCount,
                    float x,
                    float y)
 ...
 */
float interpolate3D(__global const float* table,
                    __global const float* xValues,
                    __global const float* yValues,
                    __global const float* zValues,
                    int xCount,
                    int yCount,
                    int zCount,
                    float x,
                    ...
                    float z)
{
    // Find indices (simplified for performance)
    int x0 = 0, x1 = 0;
    int y0 = 0, y1 = 0;
    int z0 = 0, z1 = 0;
...
/**
 * @brief Interpolate beam depth from cumulative depth profile
 * Matches CPU implementation in BeamDepthProfile::interpolate()
 */
float interpolateBeamDepth(__global const float* depthCumulative,
                          ...
                          float beamLocalZ)
{
    if (depthSampleCount == 0 || !isfinite(depthEntryDistance) || depthStepSize <= 0.0f) {
        return beamLocalZ;  // Fallback to geometric depth
    }

    if (!isfinite(beamLocalZ) || beamLocalZ <= depthEntryDistance) {
        return 0.0f;
    }

    const float relative = beamLocalZ - depthEntryDistance;
    if (relative <= 0.0f) {
        return 0.0f;
    }
 ...
 *
 * Kernel for computing dose at each voxel in parallel
 */
__kernel void calculateDoseKernel(
    __global const short* ctVolume,        // CT volume in HU
    __global float* doseVolume,            // Output dose volume
    __global uchar* computedMask,          // Mask of computed voxels
    ...
    // Beam data tables
    __global const float* ofTable,
    __global const float* ofDepths,
    __global const float* ofCollimators,
    int ofDepthCount,
    int ofCollimatorCount,

    __global const float* tmrTable,
    __global const float* tmrDepths,
    __global const float* tmrFieldSizes,
    int tmrDepthCount,
    int tmrFieldSizeCount,

    __global const float* ocrTable,
    __global const float* ocrDepths,
    __global const float* ocrRadii,
    __global const float* ocrCollimators,
    int ocrDepthCount,
    int ocrRadiusCount,
    int ocrCollimatorCount,

    // Depth profile for CT-density corrected depth calculation
    __global const float* depthCumulative,
    int depthSampleCount,
    float depthEntryDistance,
    float depthStepSize,
 ...
 */
__kernel void recalculateDoseWithThresholdKernel(
    __global const short* ctVolume,
    __global float* doseVolume,
    __global uchar* computedMask,
...
)
{
    ...
    // Update only if above threshold
    doseVolume[index] = dose;
 ...
 */
__kernel void interpolateVolumeKernel(
    __global float* doseVolume,
    __global const uchar* computedMask,
    int width,
    int height,
    int depth,
    int step
